...
| **5** | `year` | the year's doomsday via Odd+11, **in isolation** (R2-faithful) | "**Doomsday of {year}?**" (the weekday every doomsday date in {year} lands on) | `year = pick(rng, 1900, 2099)` | `yearDoomsdayOddEleven(year)` | **WeekdayPicker** | `gradeWeekday(…, 'yearDoom')` |
...
| **7** | `speed` | automaticity of the full solve (NO new concept), timed | identical to stage 6, back-to-back with a visible timer | `generateDate({minYear:1900, maxYear:2099}, rng)` | `weekdayOfYMD(y,m,d)`, `timed:true` | **WeekdayPicker** (timed, in the lesson drill) | `{ ...gradeProblem(...), timed: true }` |

**Why stage 5 is isolated, not GuidedSolve (refined):** R2 says a stage's drill tests *only that stage's thing*. Stage 5 teaches one atom — turning the century anchor into the year's doomsday via Odd+11 — so the drill asks for exactly that weekday ("Doomsday of 1986?") and grades it with `gradeWeekday(…, 'yearDoom')`. This keeps every lesson a single number-or-weekday answer (uniform `useLessonDrill` loop, no special multi-field GuidedSolve path). The full century→year→offset *handoff* is where it belongs: practiced end-to-end in stages 6 (`full`) and 7 (`speed`). GuidedSolve remains available as a free Practice mode after unlock.
